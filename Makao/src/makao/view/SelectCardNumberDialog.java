...
public class SelectCardNumberDialog extends JDialog {
	...
	public SelectCardNumberDialog(final BlockingQueue<MakaoActions> actionQueue){
		...
		setTitle("MAKAO WYBIERZ ŻĄDANĄ FIGURĘ");
        setResizable(false);
        setSize(500, 180);
        setLocationRelativeTo(null);
        setLayout(new GridLayout(2,0));
		String[] cardNames = { "nic nie żądam", "piątek", "szóstek", "siódemek", "ósemek", "dziewiątek", "dziesiątek" };
		final int[] values = { 10, 4, 5, 6, 7, 8, 9 };

		petList = new JComboBox(cardNames);
		petList.setSelectedIndex(0);
		add(petList);
		JButton request = new JButton("żądam");
