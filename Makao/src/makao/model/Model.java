...
public class Model {
	...
	private boolean isThatCardGood(int playerId, MakaoCard card){
		if(cardTaken){
			addMessage(TextMessage.getServerMessage(players.get(playerId).getNick(), "Nie mo˝na do∏o˝yç po pobraniu karty"));
			return false;
		}
		if(firstPlayed == null){
	...
	public void playerRequireColor(int playerId, MakaoCard card){
		...
		if (isPlayerTurn(playerId) && isThatCardGood(playerId, nc)){
			addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "˚àdamy koloru "+MakaoCard.colorToString(card.getNumber())));
			requestedColor = card;
	...
	public void playerRequireNumber(int playerId, MakaoCard card){
		...
		if (isPlayerTurn(playerId) && isThatCardGood(playerId, nc)){
			if(card.getNumber() != 10){
				addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Jopek ˝àda "+(card.getNumber()+1)));
				requestedNumber = card;
			}
			else
				addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Jopek nic nie ˝àda"));
			removeCardFromPlayer(playerId, nc);
			isEndOfGame(playerId);
		}
	}
		...
		if (isPlayerTurn(playerId)){
			if(firstPlayed == null && requestedNumber != null){
				if(card.getNumber() != requestedNumber.getNumber()){
					addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "˚àdana inna figura"));
					controller.passModelDummy(getDummy());
				}
				else{
					requestedNumber = null;
				...
				if(card.getColor() != requestedColor.getNumber() ){
					addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "˚àdany inny color"));
					controller.passModelDummy(getDummy());
				}
				else{
					requestedColor = null;
	...
	public void playerGetNextCard(int playerId){
		...
		if (isPlayerTurn(playerId)) {
			if(makaoCardToTake > 0){
				...
				addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Nie pobierasz. Zakoƒcz kolejk´ aby czekaç "+roundsToStay+" rund"));
			}
	...
	private boolean czyMakao(int playerId){
		...
		//if(!seadMakao){
			addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Nie powiedzia∏eÊ/aÊ makao\nCiàgniesz 5 kart"));
			makaoCardToTake+=5;
			seadMakao = true;
			controller.passModelDummy(getDummy());
			return true;
			...
			addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Musisz pobraç jeszcze "+makaoCardToTake+" kart"));
			return true;
		}
		return false;
	}
	...
	public void playerSayMakao(int playerId){
		addMessage(TextMessage.getServerMessage(getPlayerNick(playerId), "Makao mówi gracz"));
		seadMakao = true;
		controller.passModelDummy(getDummy());
	}
